...
private const val EMPTY_STRING_LENGTH = 0
...
fun String.htmlToText() =
    if (this.isNotEmpty()) this.replace("<(.*?)>".toRegex(),
        SPACE_STRING
    )
...
fun String?.toCapitalizeWords() =
    this?.split(SPACE_STRING)?.joinToString(
        SPACE_STRING
    ) { it.capitalize() } ?: EMPTY_STRING
...
fun String?.padLeftByValue(length: Int, valueToPad: String): String {
    val currentSize = this?.length ?: EMPTY_STRING_LENGTH
    if (currentSize >= length ||
        currentSize == EMPTY_STRING_LENGTH) {
        return this ?: ""
    }
    val result = StringBuilder()
    while (result.length < length - currentSize) {
